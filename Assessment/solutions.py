...
def simple_class(cls_var):
    '''
    Define a class (with any name you choose) with a class variable named `class_var` with value `cls_var`.

    Your class should have one instance variable called `name`

    Your class should have one method called `speak` which will return the exact string `my name is `name`'
    where `name` is an instance variable
